...
func (a *Wechat) PayNotify(ctx echo.Context) error {
	...
	if a.notifyCallback != nil {
		result := &config.Result{
			...
			TotalAmount:    payment.Round(float64(resp.GetInt64(`total_fee`))/100, 2),
...
func (a *Wechat) Refund(ctx echo.Context, cfg *config.Refund) (*config.Result, error) {
	...
	return &config.Result{
		...
		TotalAmount: payment.Round(float64(resp.GetInt64(`total_fee`))/100, 2),
		Reason:      resp.GetString(`return_msg`),
		RefundFee:   payment.Round(float64(resp.GetInt64(`refund_fee`))/100, 2),
	...
	*/
	...
	for i := int64(0); i < refundCount; i++ {
		...
		refundItems = append(refundItems, &config.RefundItem{
			Status:      refundStatus,
			RefundFee:   payment.Round(float64(refundFeeInt)/100, 2),
	...
	return &config.Result{
		...
		TotalAmount: payment.Round(float64(resp.GetInt64(`total_fee`))/100, 2),
		Reason:      resp.GetString(`return_msg`),
		RefundFee:   payment.Round(float64(refundTotalFee)/100, 2),
		RefundNo:    ``,
		OutRefundNo: ``,
